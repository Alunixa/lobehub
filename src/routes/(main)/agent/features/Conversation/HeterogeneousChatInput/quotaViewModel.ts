...
const isWeeklyAll = (row: QuotaWindowRow): boolean =>
  row.limitKind === 'weekly_all' || (row.limitKind.startsWith('weekly') && !row.scopeKey);
...
export const buildClaudeSnapshotFromWindows = (
  ...
  const session = windows.find((w) => w.limitKind === 'session');
  const weekly = windows.find(isWeeklyAll);
  const scoped = windows.find((w) => w.limitKind === 'weekly_scoped' && !!w.scopeKey);
