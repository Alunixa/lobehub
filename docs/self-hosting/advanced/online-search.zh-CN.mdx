...
可选的 SearXNG 通用网页搜索引擎优先级，使用英文逗号分隔。LobeHub 会逐个独立请求这些引擎；当前引擎遇到验证码、限流、超时、解析错误或其他上游故障时，会自动切换到下一个候选。只有全部候选引擎都失败时才返回错误。

```env
SEARXNG_ENGINE_FALLBACKS="google cse,bing,duckduckgo web,yahoo,naver"
```

LobeHub 会从 SearXNG 的 `/config` 接口读取实际快捷指令，并通过 Bang 语法精确调用单个引擎，避免默认聚合中的其他结果掩盖首选引擎故障。上述默认候选无需浏览器账号 Cookie；当前 SearXNG 实例不存在的引擎会自动跳过。
