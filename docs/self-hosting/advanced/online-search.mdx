...
## `SEARXNG_ENGINE_FALLBACKS`

Optional comma-separated priority list for SearXNG general web searches. LobeHub queries each engine independently and moves to the next one when the current engine is blocked by a CAPTCHA, rate limit, timeout, parser error, or other upstream failure. An error is returned only when every candidate engine fails.

```env
SEARXNG_ENGINE_FALLBACKS="google cse,bing,duckduckgo web,yahoo,naver"
```

LobeHub reads the actual engine shortcuts from the SearXNG `/config` endpoint and invokes engines with SearXNG bang syntax. This also prevents unrelated aggregate results from masking a failed preferred engine. The default list above does not require browser account cookies; engines that are not available on a particular SearXNG instance are skipped automatically.
