...
import type {
  ...
  NewAgentQuotaSnapshot,
  NewAgentQuotaUsageLedger,
  NewAgentQuotaWindow,
  QuotaAccountCredentials,
} from '../schemas';
import {
...
export class AgentQuotaSnapshotModel {
  ...
  append = async (rows: Omit<NewAgentQuotaSnapshot, 'userId'>[]) => {
  ...
  /** The most recent reading per (limitKind, scopeKey) for an account. */
  latestPerBucket = async (accountId: string) =>
    this.db
      .selectDistinctOn([agentQuotaSnapshots.limitKind, agentQuotaSnapshots.scopeKey])
...
export class AgentQuotaCalibrationModel {
  ...
  latest = async (accountId: string, limitKind: string, scopeKey = '') => {
    const row = await this.db.query.agentQuotaCalibrations.findFirst({
      where: and(
        ...
        eq(agentQuotaCalibrations.limitKind, limitKind),
