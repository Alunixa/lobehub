...
describe.skipIf(!RUN)('quota real-account E2E (local DB)', () => {
  ...
  it('fossilizes the live reading into account + windows on the local DB', async () => {
    ...
    for (const r of withReset) {
      const w = stored.find((x) => x.limitKind === r.limitKind && x.scopeKey === r.scopeKey);
      expect(w, `window for ${r.limitKind}/${r.scopeKey}`).toBeTruthy();
    ...
    console.log(
      ...
      stored
        .map((w) => `${w.limitKind}${w.scopeKey ? '/' + w.scopeKey : ''}=${w.peakUtilization}%`)
        .join(' '),
    );
  });
});
